            ...
            <div class="col-md-12 col-sm-12 float-sm-left">
                <div class="table-grid">
                    <table id="myTable" class="display">
                        ...
                        $query_date = $db->query( "SELECT from_date,to_date FROM `hedge_rate` WHERE quarter='q1' " );
                        $ext_date = $query_date->fetch_assoc();
                        $from_date = $ext_date['from_date'];
                        $to_date = $ext_date['to_date'];

                        //echo $users_id;
                        $sql = "SELECT c.contest_id,c.po_id,c.contest_name,c.contest_type,c.start_date,c.end_date,c.status,c.budget,po.po_number,po.po_value,po.date_of_po,po.agency_fee,po.tax,po.balance_amount,wtsr.reward_amt,wtsr.users_id,hr.hedge_rate,hr.quarter,hr.from_date,hr.to_date,u.first_name,u.last_name,u.reporting_manager FROM `contest` AS c JOIN purchase_order AS po ON c.po_id = po.po_id JOIN winner_tsr AS wtsr ON c.contest_id = wtsr.contest_id JOIN hedge_rate AS hr ON hr.hedge_id = po.hedge_id JOIN users AS u ON u.users_id = wtsr.users_id WHERE c.status = 'Ongoing' AND system_status = 'Active' AND wtsr.team_id = '$team_id' AND hr.quarter='q1' AND c.start_date >= '$from_date' AND c.end_date <= '$to_date' GROUP BY c.contest_id";
                        // $sql = "SELECT c.contest_id,c.po_id,c.contest_name,c.contest_type,c.start_date,c.end_date,c.status,c.budget,po.po_number,po.po_value,po.date_of_po,po.agency_fee,po.tax,po.balance_amount,wtsr.reward_amt,wtsr.users_id,hr.hedge_rate,hr.quarter,hr.from_date,hr.to_date FROM `contest` AS c JOIN purchase_order AS po ON c.po_id = po.po_id JOIN winner_tsr AS wtsr ON c.contest_id = wtsr.contest_id JOIN hedge_rate AS hr ON hr.hedge_id = po.hedge_id WHERE c.status = 'Ongoing' AND system_status = 'Active' AND wtsr.team_id = '$team_id' AND hr.quarter='q1' AND  c.start_date >= '$from_date' AND c.end_date <= '$to_date' GROUP BY c.contest_id";
                        $que = $db->query( $sql );
