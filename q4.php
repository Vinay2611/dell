                ...
                <div class="table-grid">
                    <table id="myTable" class="display">
                        ...
                        <tr>
                            ...
                            <th class="th-sm sorting right">Reward Amount</th>
                            <th class="th-sm sorting right">Start Date</th>
                            <th class="th-sm sorting right">End Date</th>
                            <th class="th-sm sorting left">Status</th>
                            ...
                            <th class="th-sm sorting left">Action</th>
                        </tr>
                        </thead>
                        <tbody>
                        <?php
                        $i = 1;
                        $query_date = $db->query( "SELECT from_date,to_date FROM `hedge_rate` WHERE quarter='q4' " );
                        ...
                        //$sql = "SELECT c.contest_id,c.po_id,c.contest_name,c.contest_type,c.start_date,c.end_date,c.status,c.budget,po.po_number,po.po_value,po.date_of_po,po.agency_fee,po.tax,po.balance_amount,wisr.reward_amt,hr.hedge_rate FROM `contest` AS c JOIN purchase_order AS po ON c.po_id = po.po_id JOIN winner_isr AS wisr ON c.contest_id = wisr.contest_id JOIN hedge_rate AS hr ON hr.hedge_id = po.hedge_id WHERE c.status = 'Ongoing' AND  system_status = 'Active' AND wisr.users_id = '$users_id' AND hr.quarter='q4' AND year(hr.from_date) = '$today_date' GROUP BY c.contest_id";
                        $que = $db->query( $sql );
