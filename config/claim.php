...
if(isset($_POST['type']) && $_POST['type'] == "ClaimClubRequest")
{
    ...
    if($insert) {
        ...
        //Pending
        //echo "SELECT reward_status FROM `winner` WHERE users_id='".$users_id."' AND contest_id IN(".$contest_id.") ORDER BY reward_status DESC";
        $rewardSql = $db->query("SELECT reward_status FROM `winner` WHERE users_id='".$users_id."' AND contest_id IN(".$contest_id.") ORDER BY reward_status ASC");
